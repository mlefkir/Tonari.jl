...
function test_cross_periodogram()
	...
	f, γ², γ²_corrected, Δφ, γ²_err, γ²_corrected_err, Δφ_err, Δτ, Δτ_err, P̄₁, P̄₂, N₁, N₂, n = cross_periodogram(t, x[1], x[2], σ[1], σ[2], subtract_mean = false)
...
function test_cross_periodogram_noerr()
	...
	@test length(f) == length(γ²) == length(Δφ) == length(γ²_err) == length(Δφ_err) == length(Δτ) == length(Δτ_err) == length(P̄₁) == length(P̄₂) == length(C̄)
...
function test_cross_periodogram_can_fail()
	...
	n = 1
	t, x, _ = sample(MersenneTwister(1234), sim, n)
	@test_throws "Cannot compute coherence or lags without several segments of the full time series." f, γ², Δφ, γ²_err, Δφ_err, Δτ, Δτ_err, P̄₁, P̄₂, C̄ = cross_periodogram(t, x[1], x[2])
