...
function setup_model()
    return SingleBendingPowerLaw(7.0e2, 0.13, 9.0e-2, 3.42)
end
...
function setup_model_multivariate_time()
    ...
    return cp = CrossSpectralDensity(p1, p2, Δϕ)
end
...
function setup_model_multivariate_same_psd()
    p1 = SingleBendingPowerLaw(1.0, 0.63, 10.0e-2, 3.22)
    Δϕ = ConstantPhaseLag(35.35, 2.3)
    return cp = CrossSpectralDensity(p1, p1, Δϕ)
end
...
function init_simu_regular_error()
    model = setup_model()
    T, Δt = 2.13, 334.0321
    return @test_throws "The sampling period Δt must be less than the duration T" Simulation(model, T, Δt)
end
...
function init_simu_irregular_unsorted()
    ...
    return @test_throws "The time vector must be sorted" Simulation(model, t)
end
...
function sample_simu_irregular_load()
    ...
    return @test size(x, 1) == size(σs, 1)
...
function sample_simu_irregular_extend()
    ...
    n = 3
    sim = Simulation(model, t, 15.5, 14.4)
    return tx, x, σs = sample(rng, sim, n)
end
...
function sample_simu_cross_spectral_time()
    ...
    return @test size(x[1], 1) == size(tx, 1)
end
...
function sample_simu_cross_spectral_same_psd()
    ...
    @test size(x[1]) == size(σ[1])
    @test size(x[1]) == size(σ[2])
    return @test size(x[1], 1) == size(t, 1)
end
