@doc raw"""
	fill_gaps(rng::AbstractRNG, t::Vector{Float64}, x::Vector{Float64}, randomise_values::Bool = true, poisson::Bool = true)

Fill gaps in a time series data by linear interpolation. If `randomise_values = true`, the interpolated values are drawn from a normal distribution with the mean and standard deviation of the data. If `poisson = true`, the interpolated values are drawn from a Poisson distribution with the mean of the data.
